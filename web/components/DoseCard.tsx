...
export function DoseCard({ dose, onMarkGiven, className = '' }: DoseCardProps) {
  ...
  const phoneText = dose.patient?.phoneE164 || 'No phone provided';
    ...
    <>
          ...
          <div className="space-y-1.5 flex-1">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-ink-muted">
              <span className="font-semibold text-brand">
                Dose {dose.seq} of {totalDosesCount}
              </span>
      ...
      {showFhirModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 overflow-y-auto animate-fade-in">
          <div className="bg-surface rounded-2xl border border-border shadow-2xl max-w-2xl w-full p-6 space-y-4 max-h-[90vh] flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between pb-3 border-b border-border/80">
              ...
              <button
                ...
                className="p-1 rounded-lg text-ink-muted hover:text-ink hover:bg-surfaceSunken transition-colors cursor-pointer"
            ...
            <div className="flex-1 overflow-y-auto space-y-3">
              <div className="flex items-center justify-between text-xs text-ink-muted">
                <span>Dose ID: <code className="font-mono text-ink text-[11px]">{dose.id}</code></span>
                <span className="font-mono text-[11px]">GET /fhir/Immunization/{dose.id}</span>
              </div>

              {fhirLoading && (
                <div className="p-8 text-center space-y-2">
                  <div className="w-6 h-6 border-2 border-purple-600 border-t-transparent rounded-full animate-spin mx-auto" />
                  <p className="text-xs text-ink-muted">Loading FHIR R4 standard payload...</p>
                </div>
              )}
              ...
              <div className="flex items-center gap-2">
                {fhirData && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-surface border border-border hover:bg-surfaceSunken rounded-lg transition-colors cursor-pointer"
                  >
                ...
                <button
                  type="button"
                  onClick={() => setShowFhirModal(false)}
                  className="px-4 py-1.5 text-xs font-semibold bg-brand text-white hover:bg-emerald-800 rounded-lg transition-colors cursor-pointer"
                >
                  Close
                </button>
              </div>
